    ...
    <thead>
        <tr>
            <th colspan="5" style="text-align: center; font-size:28px;padding-bottom: 20px;font-weight:800;">
                <?php echo isset($location_name) ? $location_name . ' - ' : '' ?>Doctors
            </th>
        </tr>
        <tr>
            <th style="padding: 8px;border-top:0px;border-bottom: 2px solid black;text-align: left;">Name</th>
            <th style="padding: 8px;border-top:0px;border-bottom: 2px solid black;text-align: left;">Username</th>
            <th style="padding: 8px;border-top:0px;border-bottom: 2px solid black;text-align: left;">Email</th>
        ...
        <?php if ($records) {
            foreach ($records as $row) { ?>
                <tr>
                    ...
                    <td style="padding: 8px;text-align: left;border-top: 1px solid #c8ced3;"><?php echo $row['username'] ?></td>
                    <td style="padding: 8px;text-align: left;border-top: 1px solid #c8ced3;"><?php echo $row['email'] ?></td>
                    <td style="padding: 8px;text-align: left;border-top: 1px solid #c8ced3;"><?php
                                                                                                if ($row['phone'] != null) {
                                                                                                    echo $row['phone'];
                                                                                                } else {
                                                                                                    echo "--";
