...
function custom_error_handler($number, $message){
...
class RequestMessage {    
    ...
    //<editor-fold desc="Dynamic Descriptor">
    public $DynamicDescriptorText;

    public $DynamicDescriptorAddress;

    public $DynamicDescriptorCity;

    public $DynamicDescriptorCounty;

    public $DynamicDescriptorRegion;

    public $DynamicDescriptorPostalCode;

    public $DynamicDescriptorCountryCode;

    public $DynamicDescriptorEmailAddress;
...
class BridgeCommSDKException extends Exception {
    
    //<editor-fold desc="Error Messages">
    public static function BAD_RESPONSE() { return new BridgeCommSDKError("70000", "Bad Response."); }
    public static function SERVER_ERROR() { return new BridgeCommSDKError("70001", "Could not connect to server."); }
    public static function EMPTY_NULL_FIELD(String $field) {
        
        $message = "The " . $field . " is null or empty.";

        return new BridgeCommSDKError("70002", $message);
    }    
    public static function NULL_FIELD(String $field) {

        $message = "The " . $field . " is null.";

        return new BridgeCommSDKError("70003", $message);
    }
    public static function PROCESS_REQUEST_ERROR() { return new BridgeCommSDKError("70004", "Error processing the request."); }
    public static function INVALID_RESPONSE_FORMAT() { return new BridgeCommSDKError("70005", "The BridgeComm response is not a valid xml format. Please check the URL and the request."); }
    public static function INVALID_URL_FORMAT(String $url) {

        $message = "(" . $url . ") is malformed.";

        return new BridgeCommSDKError("70006", $message);
    }
    public static function UNSUCCESSFUL_API_RESPONSE(String $apiResponse = null)
    {
        $message = "Unsuccessful response from BridgeComm.";
        if (!empty($apiResponse)) {
            $message .= " This is the BridgeComm response: " . $apiResponse;
        }
        return new BridgeCommSDKError("70007", $message);
    }
    //</editor-fold>
    
    private $bridgeCommSDKError;
    
    function __construct(BridgeCommSDKError $bridgeCommSDKError, \Throwable $previous = null) {
        parent::__construct($bridgeCommSDKError->getMessage(), $bridgeCommSDKError->getErrorCode(), $previous);
        
        $this->bridgeCommSDKError = $bridgeCommSDKError;
    }
    
    public function getBridgeCommSDKError(){ return $this->bridgeCommSDKError; }
}
// </editor-fold>


class BridgeCommConnection {
    
    const skipField = "BridgeCommResponseType";

    public function processRequest(string $bridgeCommURL, BridgeCommRequest $request){
        
        $response = new BridgeCommResponse();
        
        try {
            // <editor-fold defaultstate="collapsed" desc="Validations">
            if (empty($bridgeCommURL)) {
                
                throw new BridgeCommSDKException(BridgeCommSDKException::EMPTY_NULL_FIELD("url"));
            }
            
            if (empty($request)) {
                
                throw new BridgeCommSDKException(BridgeCommSDKException::EMPTY_NULL_FIELD("request"));
            }
            
            if (empty($request->TransactionID)) {
                
                throw new BridgeCommSDKException(BridgeCommSDKException::EMPTY_NULL_FIELD("transaction ID of request"));
            }
            
            if (empty($request->RequestType)) {
                
                throw new BridgeCommSDKException(BridgeCommSDKException::EMPTY_NULL_FIELD("type of request"));
            }
            // </editor-fold>
            ...
            $request->requestMessage->OriginatingTechnologySource = "PHPSDKv.1.1.0.0";
            ...
            if (empty($requestString)) {
                ...
                throw new BridgeCommSDKException(BridgeCommSDKException::EMPTY_NULL_FIELD("request as string"), new Exception($errorMessage));
            }
            ...
            if (empty($requestEncode)) {
                ...
                throw new BridgeCommSDKException(BridgeCommSDKException::EMPTY_NULL_FIELD("request encode"), new Exception($errorMessage));
            }
            
            $conn = new SOAPMessenger();
            $encodeResponse = $conn->sendMessage($requestEncode, $bridgeCommURL);
            
            if (empty($encodeResponse)) {
                throw new BridgeCommSDKException(BridgeCommSDKException::EMPTY_NULL_FIELD("encode response"));
            }
            ...
            if (empty($responseString)) {
                ...
                throw new BridgeCommSDKException(BridgeCommSDKException::EMPTY_NULL_FIELD("decode response"), new Exception($errorMessage));
            }
            
            $response = self::DeserializeStringXMLToObject($responseString, "BridgeCommResponse");
            
            if (empty($response)) {
                $errorMessage = "the reponse deserialization fail. This is the xml string: " . $responseString . ".";
                throw new BridgeCommSDKException(BridgeCommSDKException::EMPTY_NULL_FIELD("response"), new Exception($errorMessage));
            }
            ...
            if (!empty($tempDoc)) {
                ...
                $response->BridgeCommResponseType = $responeType;
            }
            
        } catch (BridgeCommSDKException $exc) {
            
            $errorMessage = $exc->getBridgeCommSDKError()->getMessage();
            
            if (!empty($exc->getPrevious())) {
                $errorMessage .= Constanst::ADD_MORE_DETAIL . $exc->getPrevious()->getMessage();
            }
            
            $response = $this->createErrorResponse($request, $errorMessage, $exc->getBridgeCommSDKError()->getErrorCode());
            
        }catch (Throwable $exc) {
            
            $bc_error = BridgeCommSDKException::PROCESS_REQUEST_ERROR();
    ...
    public static function DeserializeStringXMLToObject(string $stringXML, string $className){
        ...
        try {
            ...
            if ($result instanceof BridgeCommResponse) {
                $result->BridgeCommResponseType = $xmlDoc->getName();
            }
    ...
    private static function SetUpRootNode($objectToSerialize){
        ...
        if ($objectToSerialize instanceof BridgeCommRequest) {

            $rootName = "requestHeader";

        } else if ($objectToSerialize instanceof BridgeCommResponse && !empty($objectToSerialize->BridgeCommResponseType)) {

            $rootName = $objectToSerialize->BridgeCommResponseType;
    ...
    private function createErrorResponse(BridgeCommRequest $request, String $message, String $errorCode){
        ...
        try {
            ...
            if ($res->getStatusCode() == 200) {
                ...
                if (empty($bodyNode[0]) || empty($bodyNode[0]->ProcessRequestResponse) || empty($bodyNode[0]->ProcessRequestResponse[0]->ProcessRequestResult)) {
                    throw new BridgeCommSDKException(BridgeCommSDKException::UNSUCCESSFUL_API_RESPONSE($res->getBody()));
                }
                ...
                throw new BridgeCommSDKException(BridgeCommSDKException::BAD_RESPONSE(), new Exception($errorMessage));                
            }
            
        } catch (Throwable $exc) {
            throw new BridgeCommSDKException(BridgeCommSDKException::SERVER_ERROR(), $exc);
        }
