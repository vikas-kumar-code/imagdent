...
class ExistingPatients extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  clearSearch = () => {
    this.setState({ fields: {date_range:moment().format('MM-DD-YYYY') +' to '+ moment().format('MM-DD-YYYY')} }, () => {
      this.getExistingPatients(this.state.fields);
    });
  };
      ...
      if (response && params.export === true) {
        ...
        try {
          ...
          if (typeof window.navigator.msSaveBlob !== "undefined") {
            ...
            a.href = downloadUrl;
            a.download = "report-existing-patients.pdf";
  ...
  render() {
    ...
    return (
      <div className="animated fadeIn">
        <Helmet>
          <title>Existing Patients : iMagDent</title>
        </Helmet>
        <Row>
          <Col xl={12}>
            <Card>
              <CardHeader>
                <Row>
                  <Col xs={6}>
                    <Form>
                      <InputGroup>
                        <strong style={{ fontSize: 20 }}>Existing Patients</strong>
                      </InputGroup>
                    </Form>
                  </Col>
                ...
                <Search
                  ...
                  searchRecord={this.getExistingPatients}
              ...
              <CardBody>
                ...
                >
                  {this.state.patients.length > 0 && <Row>
                  ...
                  <Table responsive  className="table-striped">
                    <thead>
                      <tr>
                        <th scope="col" className="border-top-0 sortable" onClick={(event) => this.sortRecord(event, 'name')}>Name</th>
                        <th scope="col" className="border-top-0 sortable" onClick={(event) => this.sortRecord(event, 'dob')}>Birth Date</th>
                        <th scope="col" className="border-top-0 sortable" onClick={(event) => this.sortRecord(event, 'age')}>Age</th>
                        <th scope="col" className="border-top-0 sortable" onClick={(event) => this.sortRecord(event, 'phone')}>Phone</th>
                        <th scope="col" className="border-top-0 sortable" onClick={(event) => this.sortRecord(event, 'city')}>City</th>
                        <th scope="col" className="border-top-0 sortable" onClick={(event) => this.sortRecord(event, 'state')}>State</th>
                        <th scope="col" className="border-top-0 sortable" onClick={(event) => this.sortRecord(event, 'sex')}>Sex</th>
                        <th scope="col" className="border-top-0 sortable" onClick={(event) => this.sortRecord(event, 'added_on')}>Added On</th>
                      </tr>
                    </thead>
                    <tbody>
                      {this.state.patients.length > 0 ? (
                        this.state.patients.map((c, index) => (
                          <Patient
                            record={c.patient}
                            key={`key-user-${index}`}
                          />
...
export default connect(mapStateToProps)(ExistingPatients);