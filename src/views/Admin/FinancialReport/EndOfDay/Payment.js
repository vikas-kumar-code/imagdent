...
class Payment extends Component {
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <tr style={{width:"100%"}}>
          <td style={{ borderTop: `3px solid`,width:"20%" }}>
          ...
          <td style={{ borderTop: `3px solid` }}>
            {`${record.case && record.case.patient.first_name} ${
              record.case && record.case.patient.last_name
            }`}
          </td>
          ...
          <td style={{ borderTop: `3px solid`}}>
            $
            {record.amount !== null ? common.numberFormat(record.amount) : "--"}
          </td>
          <td style={{ borderTop: `3px solid`}} className="text-danger">
            <strong>${record.discount !== null ? record.discount : "0"}</strong>
          </td>
          <td style={{ borderTop: `3px solid` }}>
            $
        ...
        {record.case.services.map((cs, sindex) => (
          <tr key={`service-key-${sindex}`} className="patient-service">
            <td colSpan="5">{cs.service !== null ? cs.service.name : "--"}</td>
            <td>${cs.price !== null ? common.numberFormat(cs.price) : "0"}</td>
            <td className="text-danger">
              ${cs.discount !== null ? common.numberFormat(cs.discount) : "0"}
            </td>
            <td>
              ${cs.sub_total !== null ? common.numberFormat(cs.sub_total) : "0"}
            </td>
            <td colSpan="5">
