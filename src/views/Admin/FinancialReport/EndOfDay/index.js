...
class EndOfDay extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      search: true,
      searchFields: [
        { label: "Date Range", name: "appointment_date", type: "date-range" },
        {
          label: "Location",
          name: "location_id",
  ...
  clearSearch = () => {
    this.setState(
      ...
      () => {
        this.getEndOfDay(this.state.fields);
      }
    );
  };
  ...
  calculateGrandTotal = () => {
    ...
    let total = this.state.totalPatientAr + this.state.totalDoctorAr;
    return parseFloat(totalGt) + parseFloat(this.calculateTotalCc() + total);
  };
  ...
  totalProduction = () => {
    ...
    this.state.payments.forEach((payment, index) => {
      productionPaid[index] = parseFloat(payment.amount);
    });
    let totalAmount = 0.0;
    if (productionPaid.length > 0) {
      totalAmount = productionPaid.reduce((sum, amount) => {
        return amount + sum;
      });
    }
  ...
  totalAdjustments = () => {
    let productionPaid = [];
    this.state.payments.forEach((payment, index) => {
      productionPaid[index] = parseFloat(payment.discount);
    });
    let totalAmount = 0.0;
    if (productionPaid.length > 0) {
      totalAmount = productionPaid.reduce((sum, amount) => {
  ...
  render() {
    ...
    return (
      <div className="animated fadeIn">
        <Helmet>
          <title>End Of Day : iMagDent</title>
        </Helmet>
        <Row>
          <Col xl={12}>
            <Card>
              <CardHeader>
                <Row>
                  <Col xs={6}>
                    <Form>
                      <InputGroup>
                        <strong style={{ fontSize: 20 }}>End Of Day</strong>
                      </InputGroup>
                    </Form>
                  </Col>
              ...
              <CardBody>
                ...
                >
                {(this.state.payments.length > 0 ||  this.state.arPaymentsCollected.length > 0) && (
                    <Row>
                  ...
                  {this.state.fields["location_id"] && (
                    ...
                    >
                      <strong>
                        {this.state.fields["location_id"]["label"]} - End Of Day
                        Financial
                      </strong>
                    </Alert>
                  )}
                  ...
                  <Table responsive /*  className="table-striped" */>
                    <thead>
                      <tr>
                        ...
                        {/* <th scope="col" className="border-top-0" style={{ width: "7%" }}>
                          Case ID
                        </th> */}
                        <th scope="col" className="border-top-0" style={{ width: "9%" }}>
                          Invoice ID
                        </th>
                        <th
                          scope="col"
                        ...
                        {/* <th
                          scope="col"
                          className="border-top-0 sortable"
                          onClick={(event) => this.sortRecord(event, "payment")}
                        >
                          Payment
                        </th> */}
                        <th
                          ...
                          <tr>
                            <td>
                              ...
                              <Row
                                ...
                                >
                                  ${common.numberFormat(
                                    this.totalProduction().toFixed(2)
                                  )}
                            ...
                            <td>
                              <Row
                                ...
                                >
                                  ${common.numberFormat(
                                    this.totalAdjustments().toFixed(2)
                                  )}
                                </Col>
                              </Row>
                            </td>
                  ...
                  {this.state.payments.length > 0 && (
                          ...
                          <tr>
                            <td>
                              {this.state.groupPayments.map((gp, index) => {
                                if (parseInt(gp.mode) > 1 && parseInt(gp.mode) != 6) {
                                  return (
                                    <Row
                                      className="mb-2"
                                      key={`key-cc-${index}`}
                   ...
                   <Alert color="dark" style={{ fontSize: 25 }}>
                        ...
                        {common.numberFormat(parseInt(this.state.arPaymentsTotal).toFixed(2))}
...
export default connect(mapStateToProps)(EndOfDay);
