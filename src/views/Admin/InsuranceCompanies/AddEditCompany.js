...
class AddEditCompany extends Component {
  ...
  handleSubmit = e => {
    ...
    if (this.handleValidation()) {
      ...
      let that = this;
      company.add(params).then(response => {
  ...
  getCompany = id => {
    ...
    company.getOne({ id: id }).then(response => {
      if (response.data.success) {
        let fields = response.data.company;
        this.setState({
          loader: false,
          fields
        });
    ...
    return (
      <Modal isOpen={this.props.showModal}>
        <ModalHeader toggle={this.props.toggleModal}>
          {this.props.companyId ? "Update " : "Add "} Insurance Company
        </ModalHeader>
                ...
                <Row>
                  <Col md={12} className="text-right">
                    ...
                    >
                      ...
                      {this.props.companyId ? "Update" : "Add"}
                    </button>
                  </Col>
                </Row>
              </Form>
...
export default connect(mapStateToProps)(AddEditCompany);
